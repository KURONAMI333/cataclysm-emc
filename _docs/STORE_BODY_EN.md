...
Drop the jar on the **server** (singleplayer counts too) next to ProjectE and Cataclysm. There is no config, no command, no item: the conversion file registers with ProjectE at startup (`Considering file cataclysm:pe_custom_conversions/cataclysm_default.json` in the log) and the values simply exist.
...
2. Install [L_Ender's Cataclysm](https://modrinth.com/mod/l_enders-cataclysm) and [ProjectE](https://www.curseforge.com/minecraft/mc-mods/projecte)
...
Author: KURONAMI · Built for [L_Ender's Cataclysm](https://modrinth.com/mod/l_enders-cataclysm) by L_Ender (assets © MCL_Ender, source LGPL v3.0 — none are included here)
